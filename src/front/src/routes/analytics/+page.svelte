<script>
	...
	function renderChart() {
		...
		years.forEach((year) => {
			// @ts-ignore
			const empRecord = empData.find(
				(item) => item.year === year && item.education_level === 'TOTAL'
			);
			// @ts-ignore
			...
			seriesData.push({
				name: year.toString(),
				data: [
					empRecord && !isNaN(empRecord.employment_rate)
						? parseFloat(empRecord.employment_rate)
						: null,
...
<div class="wrapper">
	<div class="container">
		...
		<div class="graph-container">
			<h3>
				Esta gráfica integra los datos de las tres APIs. En concreto, se muestra el porcentaje de
				empleo, educación superior y cibercrimen por años.
			</h3>
